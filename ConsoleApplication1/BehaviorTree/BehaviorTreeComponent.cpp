...
{
	UBehaviorTreeManager* BTManager = UBehaviorTreeManager::GetCurrent();
	if (BTManager)
	{
...
{
	return bIsRunning && (int)InstanceStack.size();
}
...
void UBehaviorTreeComponent::StartTree(const FString& assetPath, EBTExecutionMode::Type ExecuteMode /*= EBTExecutionMode::Looped*/)
{
	UE_VLOG("BehaviorTreeComponent::StartTree  assetPath:%s\n", assetPath.c_str());
	// clear instance stack, start should always run new tree from root
	UBehaviorTree* CurrentRoot = GetRootTree();

	if (CurrentRoot && CurrentRoot->GetName() == assetPath && TreeHasBeenStarted())
	{
		UE_VLOG("Skipping behavior start request - it's already running\n");
		return;
	}
	else if (CurrentRoot)
	{
		UE_VLOG("Abandoning behavior %s to start new one (%s)\n", CurrentRoot->GetName().c_str(), assetPath.c_str());
	}

	StopTree(EBTStopMode::Safe);

	TreeStartInfo.TreePath = assetPath;
...
// #endif
	...
	// push new instance
	PushInstance(TreeStartInfo.TreePath);
...
void UBehaviorTreeComponent::StopTree(EBTStopMode::Type StopMode)
{
	...
	if (!bRequestedStop)
	{
		...
		for (int32 InstanceIndex = (int)InstanceStack.size() - 1; InstanceIndex >= 0; InstanceIndex--)
		{
			FBehaviorTreeInstance& InstanceInfo = InstanceStack[InstanceIndex];

			// notify active aux nodes
			for (int32 AuxIndex = 0; AuxIndex < (int)InstanceInfo.ActiveAuxNodes.size(); AuxIndex++)
			{
			...
			InstanceInfo.ActiveAuxNodes.clear();
			...
			//
			for (int32 ParallelIndex = 0; ParallelIndex < (int)InstanceInfo.ParallelTasks.size(); ParallelIndex++)
			{
				...
				if (CachedTaskNode && (ParallelTaskInfo.Status == EBTTaskStatus::Active))
				{
...
// 						(NodeResult == EBTNodeResult::InProgress) ? TEXT("in progress") : TEXT("instant"));
					...
					{
						const bool bIsValidForStatus = IsValidIndex(InstanceInfo.ParallelTasks, ParallelIndex) && (ParallelTaskInfo.TaskNode == CachedTaskNode);
...
// 								*UBehaviorTreeTypes::DescribeNodeHelper(CachedTaskNode));
					...
					const bool bIsValidAfterFinishing = IsValidIndex(InstanceInfo.ParallelTasks, ParallelIndex) && (ParallelTaskInfo.TaskNode == CachedTaskNode);
			...
			{
				const UBTTaskNode* TaskNode = CastNode<UBTTaskNode>(InstanceInfo.ActiveNode);
	...
	for (int32 InstanceIndex = 0; InstanceIndex < (int)InstanceStack.size(); InstanceIndex++)
	{
		// FBehaviorTreeInstance& InstanceInfo = InstanceStack[InstanceIndex];
...
{
	if (TaskNode == NULL || (int)InstanceStack.size() == 0)
	{
		return;
	}

	//UE_VLOG("UBehaviorTreeComponent::OnTaskFinished TaskNodeName:%s\n", TaskNode->GetNodeName().c_str());
...
// 		*UBehaviorTreeTypes::DescribeNodeHelper(TaskNode), *UBehaviorTreeTypes::DescribeNodeResult(TaskResult));
	...
	if (!IsValidIndex(InstanceStack, TaskInstanceIdx))
	{
		return;
	}
	...
	if (TaskResult != EBTNodeResult::InProgress)
	{
		...
		if (IsValidIndex(InstanceStack, ActiveInstanceIdx) && InstanceStack[ActiveInstanceIdx].ActiveNode == TaskNode)
		{
		...
		else if (TaskResult == EBTNodeResult::Aborted && IsValidIndex(InstanceStack, TaskInstanceIdx) && InstanceStack[TaskInstanceIdx].ActiveNode == TaskNode)
		{
...
// 		bLoopExecution ? TEXT("looping") : TEXT("stopping"));
	...
	if (bLoopExecution && (int)InstanceStack.size())
	{
...
bool UBehaviorTreeComponent::IsExecutingBranch(const UBTNode* Node, int32 ChildIndex) const
{
	...
	if (!IsValidIndex(InstanceStack, TestInstanceIdx) || InstanceStack[TestInstanceIdx].ActiveNode == NULL)
	{
		return false;
	}
...
//			CA_SUPPRESS(6011);
			if (AuxNode->IsInstanced() && TestAuxNode && TestAuxNode->GetExecutionIndex() == AuxExecutionIndex)
			{
				...
				const UBTNode* NodeInstance = TestAuxNode->GetNodeInstance(*this, (uint8*)NodeMemory);
...
{
	if (IsValidIndex(InstanceStack, InstanceIdx))
	{
		for (unsigned i = 0; i < InstanceStack[InstanceIdx].ActiveAuxNodes.size(); ++i)
		{
			if (InstanceStack[InstanceIdx].ActiveAuxNodes[i] == AuxNodeTemplate)
			{
				return true;
			}
		}
...
EBTTaskStatus::Type UBehaviorTreeComponent::GetTaskStatus(const UBTTaskNode* TaskNode) const
{
	...
	if (IsValidIndex(InstanceStack, InstanceIdx))
	{
		...
		for (int32 TaskIndex = 0; TaskIndex < (int)InstanceInfo.ParallelTasks.size(); TaskIndex++)
		{
...
void UBehaviorTreeComponent::RequestExecution(const UBTDecorator* RequestedBy)
{
	...
	const FBehaviorTreeInstance& ActiveInstance = LastOfVector(InstanceStack);
	if (ActiveInstance.ActiveNodeType == EBTActiveNode::ActiveTask)
	{
		...
		//UE_CVLOG(RelativePriority < EBTNodeRelativePriority_Same, GetOwner(), LogBehaviorTree, Error, TEXT("UBehaviorTreeComponent::RequestExecution decorator requesting restart has lower priority than Current Task"));
		//assert(RelativePriority >= EBTNodeRelativePriority_Same);
	}
...
{
	EBTNodeRelativePriority RelativePriority = EBTNodeRelativePriority_Same;
	...
	if (NodeA != NodeB)
	{
		...
		{
			RelativePriority = NodeA->GetExecutionIndex() < NodeB->GetExecutionIndex() ? EBTNodeRelativePriority_Higher : EBTNodeRelativePriority_Lower;
		}
		else
		{
			RelativePriority = (InstanceIndexA != INDEX_NONE && InstanceIndexB != INDEX_NONE) ? (InstanceIndexA < InstanceIndexB ? EBTNodeRelativePriority_Higher : EBTNodeRelativePriority_Lower)
			                   : (InstanceIndexA != INDEX_NONE ? EBTNodeRelativePriority_Higher : EBTNodeRelativePriority_Lower);
		}
	}
...
void UBehaviorTreeComponent::RequestExecution(EBTNodeResult::Type LastResult)
{
	...
	if (LastResult != EBTNodeResult::Aborted && LastResult != EBTNodeResult::InProgress && IsValidIndex(InstanceStack, ActiveInstanceIdx))
	{
		...
		RequestExecution(ExecuteParent, (int)InstanceStack.size() - 1,
		                 ActiveInstance.ActiveNode ? ActiveInstance.ActiveNode : ActiveInstance.RootNode, -1,
		                 LastResult, false);
	}
}

static void FindCommonParent(const std::vector<FBehaviorTreeInstance>& Instances, const std::vector<FBehaviorTreeInstanceId>& KnownInstances,
                             ...
                             UBTCompositeNode*& CommonParentNode, uint16& CommonInstanceIdx)
{
	...
	{
		return;
// 		FString AssetAName = IsValidIndex(Instances,InstanceIdxA) && IsValidIndex(KnownInstances,Instances[InstanceIdxA].InstanceIdIndex) ? GetNameSafe(KnownInstances[Instances[InstanceIdxA].InstanceIdIndex].TreeAsset) : TEXT("unknown");
// 		FString AssetBName = IsValidIndex(Instances,InstanceIdxB) && IsValidIndex(KnownInstances,Instances[InstanceIdxB].InstanceIdIndex) ? GetNameSafe(KnownInstances[Instances[InstanceIdxB].InstanceIdIndex].TreeAsset) : TEXT("unknown");
// 		FString AssetCName = IsValidIndex(Instances,CommonInstanceIdx) && IsValidIndex(KnownInstances,Instances[CommonInstanceIdx].InstanceIdIndex) ? GetNameSafe(KnownInstances[Instances[CommonInstanceIdx].InstanceIdIndex].TreeAsset) : TEXT("unknown");
//
...
// 		*UBehaviorTreeTypes::DescribeNodeResult(ContinueWithResult));
	...
	if (!bIsRunning || !IsValidIndex(InstanceStack, ActiveInstanceIdx))
	{
	...
	{
		ExecutionIdx.ExecutionIndex = RequestedOn->GetChildExecutionIndex(RequestedByChildIndex, EBTChildIndex_FirstNode);

		// first index outside allowed range
		LastExecutionIndex = RequestedOn->GetChildExecutionIndex(RequestedByChildIndex + 1, EBTChildIndex_FirstNode);
	}
...
//					UE_VLOG(GetOwner(), LogBehaviorTree, Log, TEXT("> skip: decorators are not allowing execution"));
	...
	else
	{
		...
		const bool bShouldCheckDecorators = IsValidIndex(RequestedOn->Children, RequestedByChildIndex) &&
		                                    (RequestedOn->Children[RequestedByChildIndex].DecoratorOps.size() > 0) &&
		                                    IsANodeType<UBTDecorator>(RequestedBy);
...
//		UE_VLOG(GetOwner(), LogBehaviorTree, Log, TEXT("> aborting current task search!"));
	...
	const bool bIsActiveNodeAborting = (int)InstanceStack.size() > 0 && LastOfVector(InstanceStack).ActiveNodeType == EBTActiveNode::AbortingTask;
...
void UBehaviorTreeComponent::ApplySearchUpdates(const std::vector<FBehaviorTreeSearchUpdate>& UpdateList, int32 NewNodeExecutionIndex, bool bPostUpdate)
{
	for (int32 Index = 0; Index < (int)UpdateList.size(); Index++)
	{
		const FBehaviorTreeSearchUpdate& UpdateInfo = UpdateList[Index];
		if (!IsValidIndex(InstanceStack, UpdateInfo.InstanceIndex))
		{
			continue;
		}
		...
		{
			bIsComponentActive = VectorContains<UBTAuxiliaryNode*>(UpdateInstance.ActiveAuxNodes, UpdateInfo.AuxNode);
		}
		else if (UpdateInfo.TaskNode)
		{
			for (unsigned i = 0; i < UpdateInstance.ParallelTasks.size(); ++i)
			{
				if (UpdateInstance.ParallelTasks[i] == UpdateInfo.TaskNode)
				{
					ParallelTaskIdx = i;
					break;
				}
...
// 			*UBehaviorTreeTypes::DescribeNodeHelper(UpdateNode));
		...
		if (UpdateInfo.AuxNode)
		{
			...
			if (bLoopExecution && UpdateInfo.AuxNode->GetMyNode() == InstanceStack[0].RootNode &&
			        IsANodeType<UBTService>(UpdateInfo.AuxNode))
			{
				if (UpdateInfo.Mode == EBTNodeUpdateMode::Remove ||
				        VectorContains<UBTAuxiliaryNode*>(InstanceStack[0].ActiveAuxNodes, UpdateInfo.AuxNode))
				{
			...
			if (UpdateInfo.Mode == EBTNodeUpdateMode::Remove)
			{
				unsigned nodeNum = UpdateInstance.ActiveAuxNodes.size();
				for (unsigned i = 0; i < nodeNum; ++i)
				{
					if (UpdateInstance.ActiveAuxNodes[i] == UpdateInfo.AuxNode)
					{
						UpdateInstance.ActiveAuxNodes[i] = UpdateInstance.ActiveAuxNodes[nodeNum - 1];
						UpdateInstance.ActiveAuxNodes.erase(UpdateInstance.ActiveAuxNodes.begin() + nodeNum - 1);
						break;
					}
				}
			...
			{
				UpdateInstance.ActiveAuxNodes.push_back(UpdateInfo.AuxNode);
...
// 					(NodeResult == EBTNodeResult::InProgress) ? TEXT("in progress") : TEXT("instant"));
				...
				const bool bStillValid = IsValidIndex(InstanceStack, UpdateInfo.InstanceIndex) &&
				                         IsValidIndex(InstanceStack[UpdateInfo.InstanceIndex].ParallelTasks, ParallelTaskIdx) &&
...
// 					*UBehaviorTreeTypes::DescribeNodeHelper(UpdateInfo.TaskNode));
				...
				UpdateInstance.ParallelTasks.push_back(FBehaviorTreeParallelTask(UpdateInfo.TaskNode, EBTTaskStatus::Active));
			}
		}
	}
}
...
void UBehaviorTreeComponent::ApplySearchData(UBTNode* NewActiveNode)
{
	...
	for (int32 Idx = 0; Idx < (int)SearchData.PendingNotifies.size(); Idx++)
	{
		const FBehaviorTreeSearchUpdateNotify& NotifyInfo = SearchData.PendingNotifies[Idx];
		if (IsValidIndex(InstanceStack, NotifyInfo.InstanceIndex))
		{
			InstanceStack[NotifyInfo.InstanceIndex].DeactivationNotify.Execute(*this, NotifyInfo.NodeResult);
		}
	}
	...
	const float CurrentFrameDeltaSeconds = GetDeltaSeconds();

	for (int32 Idx = 0; Idx < (int)SearchData.PendingUpdates.size(); Idx++)
	{
		const FBehaviorTreeSearchUpdate& UpdateInfo = SearchData.PendingUpdates[Idx];
		if (UpdateInfo.Mode == EBTNodeUpdateMode::Add && UpdateInfo.AuxNode && IsValidIndex(InstanceStack, UpdateInfo.InstanceIndex))
		{
	...
	SearchData.PendingNotifies.clear();
}

void UBehaviorTreeComponent::ApplyDiscardedSearch()
{
	std::vector<FBehaviorTreeSearchUpdate> UpdateList;
	for (int32 Idx = 0; Idx < (int)SearchData.PendingUpdates.size(); Idx++)
	{
		const FBehaviorTreeSearchUpdate& UpdateInfo = SearchData.PendingUpdates[Idx];
		if (UpdateInfo.Mode != EBTNodeUpdateMode::Remove &&
		        UpdateInfo.AuxNode && IsANodeType<UBTDecorator>(UpdateInfo.AuxNode))
		{
			...
			{
				UpdateList.push_back(UpdateInfo);
			}
		}
	}
...
void UBehaviorTreeComponent::TickComponent(float DeltaTime, ELevelTick TickType)
{
	mDeltaSeconds = DeltaTime;
	...
	for (int32 InstanceIndex = 0; InstanceIndex < (int)InstanceStack.size(); InstanceIndex++)
	{
		...
		{
			const UBTAuxiliaryNode* AuxNode = InstanceInfo.ActiveAuxNodes[AuxIndex];
			uint8* NodeMemory = NULL;// AuxNode->GetNodeMemory<uint8>(InstanceInfo);
	...
	if (InstanceStack.size() == 0 || !bIsRunning || bIsPaused)
	{
		return;
	}

	{
		...
		// tick active task
		if (IsValidIndex(InstanceStack, ActiveInstanceIdx))
		{
			FBehaviorTreeInstance& ActiveInstance = InstanceStack[ActiveInstanceIdx];
			if (ActiveInstance.ActiveNodeType == EBTActiveNode::ActiveTask ||
		...
		if (IsValidIndex(InstanceStack, ActiveInstanceIdx + 1))
		{
			FBehaviorTreeInstance& LastInstance = LastOfVector(InstanceStack);
...
//			BT_SEARCHLOG(SearchData, Verbose, TEXT("Testing node: %s"), *UBehaviorTreeTypes::DescribeNodeHelper(TestNode));
			...
			else if (ChildBranchIdx == BTSpecialChild::ReturnToParent)
			{
				...
				if (TestNode == NULL)
				{
					...
					if (ActiveInstanceIdx > 0)
					{
						...
						SearchData.PendingNotifies.push_back(FBehaviorTreeSearchUpdateNotify(ActiveInstanceIdx, NodeResult));
			...
			else if (IsValidIndex(TestNode->Children, ChildBranchIdx))
			{
				// was new task found?
...
//			UE_VLOG(GetOwner(), LogBehaviorTree, Verbose, TEXT("Search %s, reverted all changes."), !bIsSearchValid ? TEXT("is not valid") : TEXT("will be retried"));
	...
	if (!SearchData.bPostponeSearch)
	{
		...
		if (bIsSearchValid)
		{
			...
			if (LastOfVector(InstanceStack).ActiveNodeType == EBTActiveNode::ActiveTask)
			{
				AbortCurrentTask();
			}
...
void UBehaviorTreeComponent::ProcessPendingExecution()
{
	...
	if ((int)InstanceStack.size() > (ActiveInstanceIdx + 1))
	{
		for (int32 InstanceIndex = ActiveInstanceIdx + 1; InstanceIndex < (int)InstanceStack.size(); InstanceIndex++)
		{
			InstanceStack[InstanceIndex].Cleanup(*this, EBTMemoryClear::StoreSubtree);
		}

		InstanceStack.resize(ActiveInstanceIdx + 1);
	}
	...
	if (SavedInfo.NextTask && IsValidIndex(InstanceStack, ActiveInstanceIdx))
	{
...
void UBehaviorTreeComponent::RollbackSearchChanges()
{
	if (SearchData.RollbackInstanceIdx >= 0)
	{
		...
		{
			for (int32 Idx = 0; Idx < (int)InstanceStack.size(); Idx++)
			{
				//TODO_BNS
...
void UBehaviorTreeComponent::UnregisterAuxNodesInBranch(const UBTCompositeNode* Node, bool bApplyImmediately)
{
	...
	if (InstanceIdx != INDEX_NONE)
	{
		...
		std::vector<FBehaviorTreeSearchUpdate> UpdateListCopy;
...
void UBehaviorTreeComponent::ExecuteTask(UBTTaskNode* TaskNode)
{
	...
	UE_VLOG("UBehaviorTreeComponent::ExecuteTask TaskNodeName:%s\n", TaskNode->GetDescription().c_str());
	...
	for (int32 ServiceIndex = 0; ServiceIndex < (int)TaskNode->Services.size(); ServiceIndex++)
	{
		...
		ActiveInstance.ActiveAuxNodes.push_back(ServiceNode);
...
{
	const int32 CurrentInstanceIdx = (int)InstanceStack.size() - 1;
...
//	UE_VLOG(GetOwner(), LogBehaviorTree, Log, TEXT("Abort task: %s"), *UBehaviorTreeTypes::DescribeNodeHelper(CurrentTask));
	...
	if (CurrentInstance.ActiveNodeType == EBTActiveNode::AbortingTask &&
	        CurrentInstanceIdx == ((int)InstanceStack.size() - 1))
	{
...
{
	RegisterMessageObserver(TaskNode, MessageType, FAIRequestID::InvalidRequest);
}
...
void UBehaviorTreeComponent::RegisterMessageObserver(const UBTTaskNode* TaskNode, FName MessageType, FAIRequestID RequestID)
{
	...
	{
		FAIMessageInfo info;
		info.mMsgName = MessageType;
		info.mDelegate = FOnAIMessage::Create<UBTTaskNode>(const_cast<UBTTaskNode*>(TaskNode), &UBTTaskNode::ReceivedMessage);
		info.mRequestID = RequestID.GetID();
		TaskMessageObservers.push_back(info);
	}
}

void UBehaviorTreeComponent::ReceiveAIMessage(FAIMessage& msg)
{
	for (size_t i = 0; i < TaskMessageObservers.size(); ++i)
	{
		FAIMessageInfo& info = TaskMessageObservers[i];
		if (info.mMsgName == msg.MessageName)
		{
			info.mDelegate.Execute(this, msg);
		}
	}
}
...
{
	if (TaskNode && InstanceStack.size())
	{
		for (int i = (int)TaskMessageObservers.size() - 1; i >= 0; --i)
		{
			const FAIMessageInfo& info = TaskMessageObservers[i];
			if (info.mDelegate.GetObjectPtr() == static_cast<const void*>(TaskNode))
			{
				//为了不影响Observers的顺序，保持和UE4一致，直接erase
				TaskMessageObservers.erase(TaskMessageObservers.begin() + i);
			}
		}
	}
}
...
void UBehaviorTreeComponent::RegisterParallelTask(const UBTTaskNode* TaskNode)
{
	if (IsValidIndex(InstanceStack, ActiveInstanceIdx))
	{
		...
		InstanceInfo.ParallelTasks.push_back(FBehaviorTreeParallelTask(TaskNode, EBTTaskStatus::Active));
...
void UBehaviorTreeComponent::UnregisterParallelTask(const UBTTaskNode* TaskNode, uint16 InstanceIdx)
{
	...
	if (IsValidIndex(InstanceStack, InstanceIdx))
	{
		...
		for (int32 TaskIndex = (int)InstanceInfo.ParallelTasks.size() - 1; TaskIndex >= 0; TaskIndex--)
		{
...
// 					*UBehaviorTreeTypes::DescribeNodeHelper(TaskNode));
				...
				InstanceInfo.ParallelTasks.erase(InstanceInfo.ParallelTasks.begin() + TaskIndex);// , /*Count=*/1, /*bAllowShrinking=*/false);
				bShouldUpdate = true;
				break;
			}
		}
...
{
	bWaitingForAbortingTasks = (int)InstanceStack.size() ? (LastOfVector(InstanceStack).ActiveNodeType == EBTActiveNode::AbortingTask) : false;

	for (int32 InstanceIndex = 0; InstanceIndex < (int)InstanceStack.size() && !bWaitingForAbortingTasks; InstanceIndex++)
	{
		FBehaviorTreeInstance& InstanceInfo = InstanceStack[InstanceIndex];
		for (int32 TaskIndex = (int)InstanceInfo.ParallelTasks.size() - 1; TaskIndex >= 0; TaskIndex--)
		{
...
int UBehaviorTreeComponent::InitTreeBeforeStart(const FString& treePath, int iActorID, void* bb, int &iType)
{
	UBehaviorTreeManager* BTManager = UBehaviorTreeManager::GetCurrent();
	if (BTManager == NULL)
	{
	...
	UBehaviorTree* behaviorTree = BTManager->GetTreeTemplate(treePath);
	if (behaviorTree)
	{
		//init blackboardComp
		const FString& bbName = behaviorTree->GetBBName();
		if (BlackboardComp && BlackboardComp->GetBBName() != bbName)
		{
			return -1002;
		}
		if (BlackboardComp == NULL)
		{
			BlackboardComp = new UBlackboardComponent(this, bbName, mActorID, bb);

			int iRet = GetBBID(bbName);
			if (iRet <= 0)
			{
				UE_VLOG("iActorID(%d), GetBBID(%s) failed,ret(%d)\n", iActorID, bbName.c_str(), iRet);
			...
			iRet = RegistActorDataToBB(iActorID, bbName, bb);
			if (iRet != 0)
			{
				UE_VLOG("iActorID(%d), RegistActorDataToBB(%s) failed,ret(%d)\n", iActorID, bbName.c_str(), iRet);
...
bool UBehaviorTreeComponent::PushInstance(const FString& treePath)
{
...
//		UE_VLOG(GetOwner(), LogBehaviorTree, Warning, TEXT("Failed to execute tree %s: behavior tree manager not found!"), *TreeAsset.GetName());
	...
	if (ActiveParent)
	{
		uint8* ParentMemory = GetNodeMemory((UBTNode*)ActiveParent, (int)InstanceStack.size() - 1);
...
// 				*TreeAsset.GetName(), *UBehaviorTreeTypes::DescribeNodeHelper(ActiveParent));
	...
	UBehaviorTree* behaviorTree = BTManager->LoadTree(treePath, RootNode, InstanceMemorySize);
	if (behaviorTree)
	{
		...
		NewInstance.InstanceIdIndex = UpdateInstanceId(behaviorTree, ActiveNode, (int)InstanceStack.size() - 1);
		...
		const bool bFirstTime = (InstanceInfo.InstanceMemory.size() != InstanceMemorySize);
		if (bFirstTime)
		{
...
//		INC_DWORD_STAT(STAT_AI_BehaviorTree_NumInstances);
		InstanceStack.push_back(NewInstance);
		ActiveInstanceIdx = (int)InstanceStack.size() - 1;

		// start root level services now (they won't be removed on looping tree anyway)
		for (int32 ServiceIndex = 0; ServiceIndex < (int)RootNode->Services.size(); ServiceIndex++)
		{
			...
			InstanceStack[ActiveInstanceIdx].ActiveAuxNodes.push_back(ServiceNode);
			ServiceNode->WrappedOnBecomeRelevant(*this, NodeMemory);
		}
...
uint8 UBehaviorTreeComponent::UpdateInstanceId(UBehaviorTree* TreeAsset, const UBTNode* OriginNode, int32 OriginInstanceIdx)
{
	...
	// build path from origin node
	{
		...
		InstanceId.Path.push_back(ExecutionIndex);
	}
	...
	for (int32 InstanceIndex = 0; InstanceIndex < (int)KnownInstances.size(); InstanceIndex++)
	{
	...
	// add new one
	InstanceId.FirstNodeInstance = NodeInstances.size();

	KnownInstances.push_back(InstanceId);
	const int32 NewIndex = (int)KnownInstances.size() - 1;
	assert(NewIndex < MAX_uint8);
	return NewIndex;
}
...
int32 UBehaviorTreeComponent::FindInstanceContainingNode(const UBTNode* Node) const
{
	...
	if (TemplateNode && (int)InstanceStack.size())
	{
...
UBTNode* UBehaviorTreeComponent::FindTemplateNode(const UBTNode* Node) const
{
	...
	for (int32 ChildIndex = 0; ChildIndex < (int)ParentNode->Children.size(); ChildIndex++)
	{
	...
	for (int32 ServiceIndex = 0; ServiceIndex < (int)ParentNode->Services.size(); ServiceIndex++)
	{
...
{
	return IsValidIndex(InstanceStack, InstanceIdx) ? (uint8*)Node->GetNodeMemory<uint8>(InstanceStack[InstanceIdx]) : NULL;
}
...
void UBehaviorTreeComponent::RemoveAllInstances()
{
	...
	for (int32 Idx = 0; Idx < (int)KnownInstances.size(); Idx++)
	{
		const FBehaviorTreeInstanceId& Info = KnownInstances[Idx];
		if (Info.InstanceMemory.size())
		{
...
// 	{
// 		const FBehaviorTreeInstance& InstanceInfo = InstanceStack[InstanceIndex];
// 		for (int32 ParallelNodeIndex = 0; ParallelNodeIndex < (int)InstanceInfo.ParallelTasks.size(); ParallelNodeIndex++)
// 		{
...
{
	TypeCooldownTagMap::const_iterator it = CooldownTagsMap.find(CooldownTag);
	if (it != CooldownTagsMap.end())
	{
		const float CooldownEndTime = it->second;
...
void UBehaviorTreeComponent::AddCooldownTagDuration(FGameplayTag CooldownTag, float CooldownDuration, bool bAddToExistingDuration)
{
	...
	{
		TypeCooldownTagMap::iterator it = CooldownTagsMap.find(CooldownTag);
		if (it != CooldownTagsMap.end() && bAddToExistingDuration)
		{
			it->second += CooldownDuration;
		}
		else
		{
			CooldownTagsMap[CooldownTag] = (FBTTime::GetTimeSeconds() + CooldownDuration);
		}
	}
}
//
...
//
// 	for (int32 Idx = 0; Idx < TestComposite->Children.size(); Idx++)
// 	{
...
//
// 		if (InstanceInfo.ActiveAuxNodes.size() > 0)
// 		{
...
// 		}
//
// 		std::vector<FString> Descriptions;
...
// 		}
//
// 		for (int32 DescriptionIndex = Descriptions.size() - 1; DescriptionIndex >= 0; DescriptionIndex--)
// 		{
...
// 	CurrentStep.StepIndex = DebuggerSteps.size() ? DebuggerSteps.Last().StepIndex + 1 : 0;
...
// 	}
//
// 	for (int32 InstanceIndex = RemovedInstances.size() - 1; InstanceIndex >= 0; InstanceIndex--)
// 	{
...
// 	while (DebuggerSteps.size() >= ManagerCDO->MaxDebuggerSteps)
// 	{
...
//
// 		// add aux nodes
// 		for (int32 NodeIndex = 0; NodeIndex < ActiveInstance.ActiveAuxNodes.size(); NodeIndex++)
// 		{
// 			InstanceInfo.AdditionalActiveNodes.Add(ActiveInstance.ActiveAuxNodes[NodeIndex]->GetExecutionIndex());
// 		}
//
// 		// add active parallels
// 		for (int32 TaskIndex = 0; TaskIndex < ActiveInstance.ParallelTasks.size(); TaskIndex++)
// 		{
...
// 	{
// 		for (int32 FlowIndex = 0; FlowIndex < CurrentSearchFlow[InstanceIdx].size(); FlowIndex++)
// 		{
...
//
// 		if (CurrentSearchFlow.size() < (InstanceIdx + 1))
// 		{
// 			CurrentSearchFlow.SetNum(InstanceIdx + 1);
// 		}
//
// 		if (CurrentSearchFlow[InstanceIdx].size() == 0 || CurrentSearchFlow[InstanceIdx].Last().ExecutionIndex != FlowInfo.ExecutionIndex)
// 		{
...
//
// 		if (CurrentRestarts.size() < (InstanceIdx + 1))
// 		{
...
// }
//
// void UBehaviorTreeComponent::StoreDebuggerRuntimeValues(std::vector<FString>& RuntimeDescriptions, UBTNode* RootNode, uint16 InstanceIdx) const
// {
// #if USE_BEHAVIORTREE_DEBUGGER
// 	if (!InstanceStack.IsValidIndex(InstanceIdx))
// 	{
...
// 	const FBehaviorTreeInstance& InstanceInfo = InstanceStack[InstanceIdx];
//
// 	std::vector<FString> RuntimeValues;
...
// 		for (int32 ValueIndex = 0; ValueIndex < RuntimeValues.size(); ValueIndex++)
// 		{
// 			if (ComposedDesc.Len())
// 			{
...
// 			InstanceIdx, CurrentStep.InstanceStack[InstanceIdx].RuntimeDesc.size());
// 	}
// #endif
// }
//
