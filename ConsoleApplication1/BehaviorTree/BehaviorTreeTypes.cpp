...
{
	for (int32 ServiceIndex = 0; ServiceIndex < (int)Node.Services.size(); ServiceIndex++)
	{
	...
	// UBTCompositeNode* InstancedComposite = CastNode<UBTCompositeNode>(Node.GetNodeInstance(OwnerComp, NodeMemory));
	Node.GetNodeInstance(OwnerComp, NodeMemory);
// 	if (InstancedComposite)
// 	{
// 		InstancedComposite->InitializeComposite(Node.GetLastExecutionIndex());
// 	}

	for (int32 ChildIndex = 0; ChildIndex < (int)Node.Children.size(); ChildIndex++)
	{
		...
		for (int32 DecoratorIndex = 0; DecoratorIndex < (int)ChildInfo.Decorators.size(); DecoratorIndex++)
		{
			...
			UBTDecorator* InstancedDecoratorOb = CastNode<UBTDecorator>(DecoratorOb->GetNodeInstance(OwnerComp, DecoratorMemory));
		...
		else if (ChildInfo.ChildTask)
		{
			for (int32 ServiceIndex = 0; ServiceIndex < (int)ChildInfo.ChildTask->Services.size(); ServiceIndex++)
			{
				...
				UBTService* InstancedServiceOb = CastNode<UBTService>(ServiceOb->GetNodeInstance(OwnerComp, ServiceMemory));
...
void FBehaviorTreeInstance::Cleanup(UBehaviorTreeComponent& OwnerComp, EBTMemoryClear::Type CleanupType)
{
	...
	{
		const int32 MaxAllowedIdx = OwnerComp.NodeInstances.size();
		const int32 LastNodeIdx = IsValidIndex(OwnerComp.KnownInstances, InstanceIdIndex + 1) ?
...
bool FBehaviorTreeInstance::HasActiveNode(uint16 TestExecutionIndex) const
{
	...
	for (int32 Idx = 0; Idx < (int)ParallelTasks.size(); Idx++)
	{
	...
	for (int32 Idx = 0; Idx < (int)ActiveAuxNodes.size(); Idx++)
	{
...
// 		*UBehaviorTreeTypes::DescribeNodeHelper(UpdateInfo.AuxNode ? (UBTNode*)UpdateInfo.AuxNode : (UBTNode*)UpdateInfo.TaskNode));
	...
	for (int32 UpdateIndex = 0; UpdateIndex < (int)PendingUpdates.size(); UpdateIndex++)
	{
		...
		if (Info.AuxNode == UpdateInfo.AuxNode && Info.TaskNode == UpdateInfo.TaskNode)
		{
			...
			PendingUpdates.erase(PendingUpdates.begin() + UpdateIndex);
		}
	}
	...
	if (!bSkipAdding)
	{
		PendingUpdates.push_back(UpdateInfo);
		PendingUpdates[PendingUpdates.size() - 1].bPostUpdate = (UpdateInfo.Mode == EBTNodeUpdateMode::Add) && (CastNode<UBTService>(UpdateInfo.AuxNode) != NULL);
	}
	else
	{
...
// 				{
// 					bFilterPassed = false;
// 					for (int32 FilterIndex = 0; FilterIndex < AllowedTypes.size(); FilterIndex++)
// 					{
...
const FVector FVector::InvalidVector(MAX_flt);

const FRotator FRotator::InvalidRotator(MAX_flt);
//
// void FAIMessage::Broadcast(const FAIMessage& Message)
// {
//
// }

const uint32 FAIRequestID::InvalidRequestID = uint32(-1);
const uint32 FAIRequestID::AnyRequestID = 0;
...
{
	return (TreeAsset->GetName() == Other.TreeAsset->GetName()) && EqualVector<uint16>(Path, Other.Path);
}
