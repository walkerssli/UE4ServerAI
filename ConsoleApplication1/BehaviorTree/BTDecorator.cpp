...
UBTDecorator::UBTDecorator() : UBTAuxiliaryNode()
{
	NodeType |= (uint32)BTNodeTypes_Decorator;
...
uint32 UBTDecorator::sNodeTypeFlag = (uint32)BTNodeTypes_Decorator;
...
void UBTDecorator::ConditionalFlowAbort(UBehaviorTreeComponent& OwnerComp, EBTDecoratorAbortRequest RequestMode) const
{
	...
	const bool bPass = WrappedCanExecute(OwnerComp, NULL);
	const bool bAbortPending = OwnerComp.IsAbortPending();
	const bool bAlwaysRequestWhenPassing = (RequestMode == ConditionPassing);
...
// 		bLogRestart ? TEXT("restart") : TEXT("skip"));
	...
	else if (!bIsExecutingBranch && !bPass && GetParentNode() && IsValidIndex(GetParentNode()->Children, GetChildIndex()))
	{
...
// FXmlNode* UBTDecorator::ExportXml(class FXmlNode* ParentNode, class FXmlNode* TypesNode, TSet<UBehaviorTree*>& SubTrees) const
// {
// 	BTXmlUtils::ExportEnumXml(TEXT("EBTFlowAbortMode"), TypesNode);
//
// 	FXmlNode* Node = Super::ExportXml(ParentNode, TypesNode, SubTrees);
//
// 	Node->AppendAttribute(TEXT("bInverseCondition"), BTXmlUtils::GetBoolString(bInverseCondition));
// 	Node->AppendAttribute(TEXT("FlowAbortMode"), BTXmlUtils::GetEnumString(TEXT("EBTFlowAbortMode"), (int32)FlowAbortMode));
// 	Node->AppendAttribute(TEXT("FlowAbortModeValue"), FString::FromInt((int32)FlowAbortMode));
//
// 	return Node;
// }
//#endif // endif
